use std::{path::{Path, PathBuf}, fs::File, time::Duration, sync::Arc};
use aws_sdk_s3::{Client, Region, types::ByteStream};
use aws_sdk_s3::error::PutObjectError;
use aws_sdk_s3::operation::put_object::PutObjectErrorKind;
use aws_config::meta::region::RegionProviderChain;
use aws_smithy_types_convert::date_time::DateTimeExt;
use clap::Parser;
use walkdir::WalkDir;
use tokio::{io::AsyncReadExt, time::sleep, sync::Semaphore, task::JoinSet};
...
async fn main() -> Result<()> {
    ...
    async fn send_otel_telemetry(endpoint: &str, payload: &serde_json::Value) -> Result<()> {
        ...
        send_otel_telemetry_retry(&client, endpoint, payload).await
    }

    async fn send_otel_telemetry_retry(client: &HttpClient, endpoint: &str, payload: &serde_json::Value) -> Result<()> {
        for attempt in 1..=3 {
            let res = client.post(endpoint).json(payload).send().await;
            match res {
                Ok(r) if r.status().is_success() => return Ok(()),
                Ok(r) => {
                    warn!("OTEL attempt {}/3 failed: {}", attempt, r.status());
                    sleep(Duration::from_secs(2)).await;
                }
                Err(e) => {
                    warn!("OTEL attempt {}/3 error: {}", attempt, e);
                    sleep(Duration::from_secs(2)).await;
                }
            }
        }
        Err(anyhow::anyhow!("OTEL telemetry failed after retries"))
        }
    ...
    log_config.set_time_format_str("%Y-%m-%d %H:%M:%S");
...
CombinedLogger::init(vec![
    TermLogger::new(level, log_config.clone(), TerminalMode::Stdout, ColorChoice::Auto),
...
use aws_smithy_client::timeout::TimeoutConfig;
    ...
    for entry in WalkDir::new(&source_path).into_iter().filter_map(Result::ok).filter(|e| e.file_type().is_file()) {
        ...
        let rel_path = full_path.strip_prefix(&source).unwrap().to_string_lossy().replace("\\", "/");
        ...
        join_set.spawn(async move {
            ...
            let _permit = permit;
            if dry_run {
                info!("[DRY RUN] Would upload {} to s3://{}/{}", full_path.display(), bucket, key);
            ...
            if metadata1 != metadata2 || has_open_writers(&full_path)? {
                warn!("Skipping file: {} — currently being written or has open file descriptors", full_path.display());
                return Ok(());
            }
                ...
                match upload_file(&client, &bucket, &key, &full_path).await {
                    Ok(_) => {
                        ...
                        if let (Some(client), Some(url)) = (otel_client.as_ref(), otel_url.as_ref()) {
                            ...
                            if let Err(e) = send_otel_telemetry_retry(client, url, &payload).await {
                    ...
                    Err(e) => {
                        warn!("Attempt {}/{} failed for {}: {}", attempt, max_retries, key, e);
    ...
    info!("Upload complete: {} files attempted, {} failed.", total, failed);
    info!("Summary: {} uploaded successfully, {} skipped or failed.", total - failed, failed);
                        ...
                        if let Ok(link) = fs::read_link(fd.path()) {
                            if let Ok(meta) = fs::metadata(&link) {
                                if meta.ino() == target_ino {
                                    info!("Open FD on file {} by PID {}", path.display(), pid_str);
                                    return Ok(true);
                                }
                            }
                        }
...
async fn upload_file(client: &Client, bucket: &str, key: &str, path: &Path) -> Result<(), PutObjectError> {
...
mod tests {
    ...
    fn test_writer_check_open_fd() {
        ...
        assert!(matches!(result_open, Ok(true)), "Expected open file to report true");

        drop(temp_file); // close file
        let result_closed = has_open_writers(&path);
        assert!(matches!(result_closed, Ok(false)), "Expected closed file to report false");
    }
}
